...
def plot_mod(mod, root="results/eq_compare"):
    curves=[]
    for eq in ["zf","mmse"]:
    ...
    plt.title(f"{mod}: ZF vs MMSE (taps=8, pilots=8, interp=FFT)")
    plt.legend()
    outpng=os.path.join(root, mod, f"overlay_{mod}_ZF_vs_MMSE.png")
